...
if algoritmo == "Punto Fijo":
    ...
    with st.form("form_punto_fijo"):
        ...
        with col1:
            ...
            expr_g = st.text_input("g(x)  (dejar vacio para usar g(x) = x - f(x))", value="")
    ...
    if ejecutar_pf or "resultado_punto_fijo" in st.session_state:
        try:
            if ejecutar_pf:
                ...
                g_auto = False
                if not expr_g.strip():
                    expr_g_usado = f"x - ({normalizar_expresion(expr_f)})"
                    g_auto = True
                else:
                    expr_g_usado = expr_g
...
st.markdown("---")
st.caption("Proximo paso: agregar Newton-Raphson, Secante, Falsa Posicion.")
