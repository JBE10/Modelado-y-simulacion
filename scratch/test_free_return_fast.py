...
def run_simulation(physics, payload_mass, stage3_prop, start_angle=0.0, t_ballistic=800000):
    ...
    while t < t_final:
        curr_dt = 50.0 if state[1] >= 120000.0 else h_step
...
def search():
    ...
    print("Searching for trajectories that reach Moon orbit and return to Earth...", flush=True)
    # Check propellant values between 26000 and 32000 in steps of 1000
    for prop in [26850, 28000, 29000, 30000, 31000, 32000]:
        print(f"--- Testing propellant: {prop} kg ---", flush=True)
        # Scan 72 angles (5 degrees increments)
        angles = np.linspace(0, 2*np.pi, 72, endpoint=False)
        for ang in angles:
            min_d, max_alt, returned, final_state, sim_t = run_simulation(physics, payload, prop, start_angle=ang, t_ballistic=800000)
            
            # Print if it returned to earth AND reached at least 250,000 km altitude (high orbit)
            if returned and max_alt > 250000000.0:
                print(f"  MATCH: Ang={ang:.4f} rad ({np.degrees(ang):.1f} deg) | "
