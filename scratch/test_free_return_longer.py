...
def run_simulation(physics, payload_mass, stage3_prop, start_angle=0.0, t_ballistic=1500000):
...
def search():
    ...
    print("Searching with t_ballistic = 1,500,000s...", flush=True)
    # Check propellants 30000 and 32000
    for prop in [30000, 32000]:
        print(f"--- Testing propellant: {prop} kg ---", flush=True)
        # Scan 72 angles
        angles = np.linspace(0, 2*np.pi, 72, endpoint=False)
        for ang in angles:
            min_d, max_alt, returned, final_state, sim_t = run_simulation(physics, payload, prop, start_angle=ang, t_ballistic=1500000)
            
            # Print if max altitude > 250,000 km
