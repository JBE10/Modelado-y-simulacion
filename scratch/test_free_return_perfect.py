...
def run_simulation(physics, payload_mass, stage3_prop, start_angle=0.0, t_ballistic=2000000):
    ...
    while t < t_final:
        curr_dt = 100.0 if state[1] >= 120000.0 else h_step
...
def search():
    ...
    print("Searching with t_ballistic = 2,000,000s in critical range...", flush=True)
    # Check propellants from 27000 to 29500
    for prop in [27000, 27500, 28000, 28500, 29000]:
        print(f"--- Testing propellant: {prop} kg ---", flush=True)
        # Scan 72 angles
        angles = np.linspace(0, 2*np.pi, 72, endpoint=False)
        for ang in angles:
            min_d, max_alt, returned, final_state, sim_t = run_simulation(physics, payload, prop, start_angle=ang, t_ballistic=2000000)
            
            # Print if it reached trans-lunar heights (>250k km)
            if max_alt > 250000000.0:
                print(f"  Ang={ang:.4f} rad ({np.degrees(ang):.1f} deg) | "
                      f"Min Moon Dist={min_d/1000.0:.1f} km | Max Alt={max_alt/1000.0:.1f} km | "
                      f"Returned={returned} | Final Alt={final_state[1]/1000.0:.1f} km | "
                      f"Flight Time={sim_t:.0f} s", flush=True)
