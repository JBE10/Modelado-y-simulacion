...
def run_simulation(physics, t_ballistic=300000, start_angle=0.0):
    # Standard RK4 step
    ...
    # Artemis 2 configuration
    payload_mass = 26520
    stages = [
        {"name": "Boosters + Core", "T": 39100000, "m_p": 1600000, "m_d": 293000, "md": 12000},
        {"name": "Core Stage", "T": 9000000, "m_p": 600000, "m_d": 98000, "md": 2000},
        {"name": "ICPS", "T": 110100, "m_p": 26850, "m_d": 3800, "md": 24}
    ]
    
    # Custom initial angle (could help align with the Moon)
    # The default starting x is 0, which corresponds to phi = 0
    t = 0.0
    state = np.array([start_angle * physics.Re, 0.0, 0.0, 0.0]) # starting position
    h_step = 0.5
...
def test():
    physics = TestPhysics()
    # Try different starting positions to see if we can get close to the Moon
    print("Testing different start angles...")
    for start_ang in [0.0, 0.5, 1.0, 1.5, 2.0, 2.5, 3.0, 3.5, 4.0]:
        t_arr, z_arr, d_moon_arr, final_state = run_simulation(physics, t_ballistic=600000, start_angle=start_ang)
        min_d_moon = np.min(d_moon_arr) / 1000.0 # to km
        max_alt = np.max(z_arr) / 1000.0 # to km
        print(f"Start Angle: {start_ang:.1f} rad | Max Alt: {max_alt:.1f} km | Min Dist to Moon: {min_d_moon:.1f} km | Final Alt: {final_state[1]/1000:.1f} km")
