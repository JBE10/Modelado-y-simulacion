...
\textbf{Filtro de Kalman}   & \textbf{0.011802} & \textbf{11.93} \\
\bottomrule
\end{tabular}
\end{center}
...
La Figura \ref{fig:comparacion} muestra la señal original, la ruidosa y la reconstrucción lograda por cada método. Se observa que el filtro de Kalman logra una estimación mucho más fiel a la dinámica oscilatoria de la voz.

\begin{figure}[htbp]
  \centering
  \includegraphics[width=\textwidth]{fig4_voz_comparacion.png}
  \caption{Reconstrucción de señal de voz: comparación de métodos.}
  \label{fig:comparacion}
\end{figure}

Las métricas comparativas se resumen en la Figura \ref{fig:metricas}, donde se aprecia la reducción del error y la ganancia en SNR del filtro de Kalman frente a las alternativas clásicas.

\begin{figure}[htbp]
  \centering
  \includegraphics[width=0.9\textwidth]{fig6_metricas_barras.png}
  \caption{Comparativa de MSE y SNR para todos los métodos.}
  \label{fig:metricas}
\end{figure}
...
El polinomio de grado 8 presenta el peor desempeño: MSE $= 0.1693$, SNR $= 0.36$ dB.
...
exactamente el ruido, obteniendo MSE $= 0.0209$ y SNR $= 9.44$ dB, levemente inferior
...
Con $Q = R = 10^{-2}$, el filtro obtiene \textbf{MSE $= 0.0118$} y \textbf{SNR $= 11.93$ dB},
superando a todos los métodos. La mejora de $+1.89$ dB sobre la señal ruidosa y de
$+2.49$ dB sobre el spline es consecuencia directa de la estimación probabilística:
en cada paso, el filtro pondera la predicción del modelo con la observación actual
según la relación de covarianza, asignando mayor peso a la fuente más confiable.
...
\begin{enumerate}
  \item El filtro de Kalman escalar, con parámetros $Q = R = 10^{-2}$, logró
        \textbf{SNR $= 11.93$ dB}, superando la señal ruidosa ($10.04$ dB),
        el spline cúbico ($9.44$ dB) y la interpolación polinómica ($0.36$ dB).
