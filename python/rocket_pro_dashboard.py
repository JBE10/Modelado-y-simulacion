...
st.set_page_config(page_title="Mission Control Pro", layout="wide", page_icon="🚀")
...
st.markdown("Simulación avanzada de vuelo de cohetes multi-etapa con telemetría en tiempo real, física atmosférica, resistencia dinámica y órbita lunar.")
...
with st.sidebar:
    st.header("🚀 Parámetros del Cohete")
    ...
    if config:
        ...
        t_bal = config["bal"]
        h_step = st.slider("Paso físico h (s)", 0.1, 2.0, 0.5)
    else:
        with st.expander("Etapa 1 (Booster)", expanded=True):
            ...
            m_dot1 = st.number_input("Tasa de Consumo (kg/s)", value=2500)
        ...
        with st.expander("Etapa 2 (Orbital)", expanded=False):
            ...
            m_dot2 = st.number_input("Tasa de Consumo (kg/s) ", value=260)
        
        with st.expander("Carga Útil & Sim", expanded=False):
            m_payload = st.number_input("Masa de Carga Útil (kg)", value=5000)
            t_bal = st.number_input("Vuelo Balístico (s)", value=600)
            h_step = st.slider("Paso físico h (s)", 0.1, 2.0, 0.5)

    st.divider()
    st.header("🛰️ Configuración Visual")
    num_satellites = st.slider("Número de Satélites", 0, 100, 30)
    show_moon = st.checkbox("Incluir la Luna", value=True)
    camera_mode = st.radio("Modo de Cámara Inicial", ["Libre", "Cabina", "Persecución"])
    playback_speed = st.slider("Velocidad de Animación Inicial", 0.1, 5.0, 1.0, step=0.1)
    show_orbits = st.checkbox("Mostrar Órbitas de Satélites", value=True)
    
    run_rocket = st.button("🚀 INICIAR LANZAMIENTO", type="primary")

# --- Run simulation ---
# Clear simulation cache if the preset selection changes to avoid showing stale data
if "last_preset" not in st.session_state:
    st.session_state["last_preset"] = preset_choice
elif st.session_state["last_preset"] != preset_choice:
    if "sim_results" in st.session_state:
        del st.session_state["sim_results"]
    st.session_state["last_preset"] = preset_choice

# Detect if simulation parameters changed for automatic re-runs of presets
current_params = {
...
if preset_choice == "Personalizado":
    current_params.update({
        "T1": T1, "m1_prop": m1_prop, "m1_dry": m1_dry, "m_dot1": m_dot1,
        "T2": T2, "m2_prop": m2_prop, "m2_dry": m2_dry, "m_dot2": m_dot2
    })

should_run = False
if preset_choice != "Personalizado":
    # Presets run automatically if parameter changed or no results exist
    if "sim_results" not in st.session_state or st.session_state.get("last_sim_params") != current_params:
        should_run = True
else:
    # Custom rocket runs only when user explicitly clicks launch button
    if run_rocket:
        should_run = True

if should_run or "sim_results" in st.session_state:
    if should_run:
...
init();
            ...
            with st.container(border=True):
                ...
                with tab1:
                    ...
                    st.altair_chart(c_alt, use_container_width=True)
                    st.altair_chart(c_vel, use_container_width=True)
                with tab2:
                    ...
                    st.altair_chart(c_g, use_container_width=True)
                    st.altair_chart(c_q, use_container_width=True)
                with tab3:
